...
export function CmsProvider({ children, initialData }: CmsProviderProps) {
  ...
  const seed =
    ...
    (typeof window !== "undefined"
      ? (window as Record<string, unknown>).__INITIAL_CMS__ as CmsSettings | undefined
      : undefined);

  const [settings, setSettings] = useState<CmsSettings>(() => {
    if (seed) {
      // seed is already a parsed CmsSettings object from the server
      return seed as unknown as CmsSettings;
    }
